...
test("provides a branded login page backed by a secure session route", async () => {
  ...
  const content = await read("app/login/login-page-content.tsx")
  ...
  assert.match(page, /getSafeRedirectPath/)
  assert.match(page, /LoginPageContent/)
  assert.match(content, /messages\.common\.productName/)
  assert.match(content, /LanguageSwitcher/)
  assert.match(content, /LoginForm/)
  assert.match(form, /messages\.auth\.username/)
  assert.match(form, /messages\.auth\.password/)
